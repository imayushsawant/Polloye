---
version: alpha
name: Polloye-design-system
description: "An editorial live-quiz marketing and product canvas built around a soft cream-white ground, charcoal type set in Saans (geometric sans), and a single confident Sage Green (#7BA05B) reserved for Polloye brand accents. Cards live as floating white tiles with thin hairline borders and minimal radii (8–16px). Display headlines run Saans at weight 500 with measured negative tracking. The system reads as a careful, product-led publication: quiz session screenshots dominate, ornament is rare, and the only place chromatic energy enters is the Sage Green CTA."
...
Polloye's marketing and product canvas is a soft cream-white ground (`{colors.canvas}` ≈ #f5f1ec) — not pure white. The warmth is the brand's signal: this is editorial, calm, and quiz-focused, not bright SaaS. On top of the cream canvas sit white floating cards (`{colors.surface-1}`), thin hairline dividers (`{colors.hairline}`), and charcoal type (`{colors.ink}` #111111).

Display type is **Saans** — a geometric sans — set at weight 500 with measured negative letter-spacing (-2.0px on 72px display). Body type is the same family at weight 400. The mono face is **SaansMono**, used sparingly for session codes, timers, and UI chrome embedded in product screenshots.

The single chromatic accent is **Sage Green** (`{colors.sage-green}` #7BA05B) — Polloye's brand accent. It surfaces on brand CTAs (Create quiz, Begin quiz, Join), live-session highlights, and a few inline emphasis moments. It is NOT a system primary; the system primary is charcoal `{colors.ink}`. A small **report palette** (`{colors.report-blue}`, `{colors.report-green}`, `{colors.report-pink}`, `{colors.report-lime}`) is reserved for leaderboard charts and results analytics surfaces.

The page rhythm is heavy on **product mockups**: every section's payload is a high-fidelity screenshot of Polloye's quiz UI (lobby, live question, host controls, leaderboard), framed in white cards with `{rounded.xl}` 16px corners. The marketing chrome is intentionally quiet so the live quiz experience can be the protagonist.

**Key Characteristics:**
- **Cream canvas** (`{colors.canvas}` #f5f1ec) is the brand's defining surface — neither white nor gray, deliberately warm.
- Product-screenshot-led page rhythm: every section centers a quiz UI mockup card, marketing chrome stays minimal.
- **Saans** geometric sans-serif carries the entire hierarchy; SaansMono for codes and timers.
- **Charcoal** `{colors.ink}` (#111111) is the system primary — buttons, headlines, body type all sit on charcoal.
- **Sage Green** (`{colors.sage-green}` #7BA05B) is the brand accent — used on brand CTAs and live-session highlights, never decoratively.
...
> Surfaces: marketing home, quiz authoring, waiting room / lobby, live quiz (host + participant), results / leaderboard.

### Brand & Accent
- **Charcoal** ({colors.ink}): The system primary surface. Headlines, body type, primary CTA background — all charcoal.
- **White** ({colors.on-primary}): Text on charcoal CTAs; canvas of floating cards.
- **Sage Green** ({colors.sage-green}): The Polloye brand accent. Used on brand CTAs (Create / Begin / Join), live-session highlights, and a small set of inline emphasis moments.
- **Report Orange** ({colors.report-orange}): Reserved for results / analytics charts inside in-product surfaces.
- **Brand Blue** ({colors.brand-blue}): Saturated brand blue (#0007cb) — used sparingly on marketing illustrations.
...
The report palette appears INSIDE product UI mockups — leaderboard charts and results analytics — not on marketing surface colors.

## Typography

### Font Family

- **Saans** — Geometric sans, fallback `Saans Fallback, ui-sans-serif, system-ui`. Carries display, body, eyebrow, and button.
- **SaansMono** — Mono, fallback `SaansMono Fallback, ui-monospace`. Used for session codes, timers, and code-like UI in product mockups.
...
| `{typography.headline}` | 28px | 500 | 1.20 | -0.5px | Quiz titles, CTA banner |
| `{typography.card-title}` | 22px | 500 | 1.25 | -0.3px | Card title, question card |
| `{typography.subhead}` | 20px | 400 | 1.40 | -0.2px | Lead body, intro paragraphs |
| `{typography.body-lg}` | 18px | 400 | 1.50 | -0.1px | Hero subhead, question prompts |
| `{typography.body}` | 16px | 400 | 1.50 | 0 | Default body |
| `{typography.body-sm}` | 14px | 400 | 1.50 | 0 | Card body, footer, meta |
| `{typography.caption}` | 12px | 400 | 1.40 | 0 | Captions, scores meta |
| `{typography.button}` | 15px | 500 | 1.20 | 0 | Button labels |
| `{typography.eyebrow}` | 14px | 500 | 1.30 | 0 | Section eyebrow (sentence case) |
| `{typography.mono}` | 13px | 400 | 1.50 | 0 | SaansMono for session codes / timers |
...
- Card interior padding: `{spacing.lg}` 24px on quiz/feature cards; `{spacing.xl}` 32px on testimonial cards; `{spacing.xxl}` 48px on CTA banners.
...
- Quiz list / feature grids are 3-up; live question options sit 2×2 where appropriate.
...
| 1 (lift on cream) | `{colors.surface-1}` white background on `{colors.canvas}` cream | Quiz cards, feature cards, product mockups |
...
Polloye resists drop shadows. Depth is communicated by the white-on-cream surface change.

### Decorative Depth

- **Quiz UI mockups** dominate every section's right column or center band — these are screenshots of lobby, live questions, and leaderboards, not illustrations.
...
| `{rounded.lg}` | 12px | Quiz cards, feature cards, FAQ rows |
...
**`button-accent`** — Sage Green CTA — reserved for Polloye brand actions (Create quiz, Begin quiz, Join).
- Background `{colors.sage-green}`, text `{colors.on-primary}`, type `{typography.button}`, rounded `{rounded.md}`, padding 10px 18px.

### Segment Tabs

**`pricing-tab-default`** + **`pricing-tab-selected`** — Pill-toggle for segmented controls (e.g. My quizzes / Shared).
...
**`pricing-card`** — Quiz list / plan-style summary card.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`pricing-card-featured`** — Featured / highlighted quiz card — inverts to charcoal.
- Background `{colors.ink}`, text `{colors.on-primary}`, otherwise identical structure.

**`feature-card`** — Generic feature highlight (live sessions, speed scoring, share codes).
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`product-mockup-card`** — The dominant card type — frames a high-fidelity quiz UI screenshot.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.xl}`, padding 24px.

**`testimonial-card`** — Host / educator quote with avatar + name + role.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body-lg}`, rounded `{rounded.lg}`, padding 32px.

**`startup-discount-card`** — Tinted promo / callout card on `{colors.surface-2}`.
- Background `{colors.surface-2}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 32px.

**`customer-logo-tile`** — Small tile in a partner / classroom marquee.
- Background `{colors.canvas}`, text `{colors.ink-muted}`, type `{typography.caption}`, rounded `{rounded.xs}`, padding 16px.

**`cta-banner`** — Closing CTA panel near page bottom (e.g. Start a live quiz).
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.headline}`, rounded `{rounded.lg}`, padding 48px.

### Inputs & Forms

**`text-input`** + **`text-input-focused`** — Form fields for quiz titles, join codes, and auth.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.md}`, padding 10px 14px.

### FAQ

**`faq-row`** — Expandable accordion row in marketing FAQ.
...
**`top-nav`** — Sticky cream bar with the Polloye wordmark left, nav links centered, log-in + sign-up pair right.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-sm}`, height 56px.

### Footer

**`footer`** — Dense link grid on `{colors.canvas}` cream with the Polloye wordmark left.
...
- Use **`button-accent`** Sage Green ONLY on brand CTAs (Create / Begin / Join) and live-session highlights.
...
- Don't use Sage Green as a section background or as a generic primary CTA (charcoal stays primary).
...
| Mobile-Lg | 768px | Quiz lists become accordion; nav hamburger |
| Mobile | 480px | Single-column; display-xl scales 72px → ~32px |

### Touch Targets

- CTAs hold ≥40px tap height across viewports.
- Segment tab pills hold ≥40px tap height.
- Form inputs and answer options hold ≥44px tap target on touch.

### Collapsing Strategy

- **Top nav**: links collapse to hamburger below 768px; primary CTA stays visible.
- **Card grids**: 3-up → 2-up at 1024px → 1-up below 768px.
- **Live question options**: 2×2 desktop may stack to 1 column on narrow phones.
- **Display type**: `{typography.display-xl}` 72px scales toward `{typography.display-md}` 40px on mobile.

### Image Behavior

- Quiz UI screenshots maintain aspect ratio and never crop.
- Partner logos in the marquee may collapse from 6-up to 3-up below 768px.
...
6. Treat Sage Green as a brand accent: Create / Begin / Join CTAs and live-session highlights only.
7. Lead every section with a quiz UI screenshot.

## Known Gaps

- The **report palette** lives in leaderboard / results charts rendered inside product surfaces; they are documented for completeness but are not brand surface colors.
- Form-field error and validation styling should reuse `{colors.semantic-error}` but is not fully specified yet.
- Dark mode is not documented — Polloye ships a light cream canvas only.
- Live host / participant surfaces have interaction states (selected option, revealed answer, timer urgency) that aren't fully enumerated as marketing chrome.
- Saans and SaansMono may need an open-source substitute (Inter, Söhne, Geist / JetBrains Mono) in implementation.
